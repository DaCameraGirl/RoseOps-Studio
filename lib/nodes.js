...
const { LLM_PROVIDERS, runLlmChat } = require("./llm");
...
function createNodeRegistry(credentials) {
  ...
  const nodeTypes = {
    ...
    llm: {
      ...
      config: [
        { key: "provider", label: "Provider", type: "select", options: ["openai", "google", "deepseek", "xai", "anthropic", "azure", "opencode", "ollama"], default: "openai" },
        { key: "credentialId", label: "API Key", type: "credential", credentialTypes: ["openai_api", "google_gemini", "deepseek_api", "xai_grok", "anthropic_api", "azure_openai", "opencode_zen", "ollama_local", "bearer_token"], default: "" },
        ...
        { key: "userPrompt", label: "User prompt", type: "code", default: "{{message}}" },
        { key: "temperature", label: "Temperature", type: "number", default: 0.7 },
      ],
      validate(node) {
        const errs = [];
        if (!node.config?.credentialId) errs.push("API key credential is required — add one in Secrets vault");
        if (!node.config?.userPrompt?.trim()) errs.push("User prompt is required");
        const provider = node.config?.provider || "openai";
        if (!LLM_PROVIDERS[provider]) errs.push(`Unknown provider: ${provider}`);
        return errs;
      },
      async execute(node, input) {
        const cfg = node.config || {};
        const cred = await resolveCredential(node, "credentialId");
        const systemPrompt = interpolateTemplate(cfg.systemPrompt || "", input || {});
        const userPrompt = interpolateTemplate(cfg.userPrompt || "", { ...input, message: input?.message || input?.text || JSON.stringify(input || {}) });
        ...
        return { ...input, response: result.text, llm: result };
      },
    },
    googleSheets: {
      name: "Google Sheets",
